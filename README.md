...
* **[NEW] 버프 오버레이 기능**: 프로필별로 설정 가능한 버프 오버레이 기능을 제공합니다.
...
> **💡 팁:** 버전 업데이트 시 기존 환경설정 유지 방법은 `d2_overlay_config.json` 파일과 `profiles` 폴더를 재사용 하시면 됩니다.
...
* **[NEW] Buff Overlay Feature:** Provides customizable buff overlay functions with profile support.
...
> **💡 Tip:** To keep your previous settings and profiles when updating to a new version, simply reuse your existing `d2_overlay_config.json` file and the `profiles` folder.
