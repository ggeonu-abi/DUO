...
* **실시간 공역 알림:** 매시 정각 및 30분마다 업데이트되는 공역 정보를 한글로 표시합니다. (ACT 정보 포함)
...
### 3. 실행 및 설정 방법
1.  압축을 푼 폴더 안에 `d2_tz.exe` 파일과 **`area.json`** 파일이 함께 있는지 확인합니다. (`area.json`은 공역 이름의 한글 번역 데이터를 불러오는 데 꼭 필요합니다.)
...
6.  다시 아이콘을 우클릭하여 **`⚙️ 환경설정`** 에 들어가면 래더/하드코어 모드 설정, 반투명 배경, 티어, 언어, 즐겨찾기 관리 등 다양한 개인 맞춤 옵션을 설정할 수 있습니다. (설정값은 `d2_overlay_config.json`에 자동 저장됩니다.)
...
6. Right-click the icon again and go to **`⚙️ Settings`** to customize your modes (Ladder/NL, HC/SC), translucent background, favorite zones, alerts, and more. (Settings are auto-saved to `d2_overlay_config.json`).
